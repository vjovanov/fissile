...
- **Errors point at the file.** Every line diagnostic leads with `<path>: <actual> <counting basis> (budget <limit>)`; byte and token diagnostics retain `<path>: <actual> <unit>` (§FS-004-check-audit). Editors and agents jump to the path unmodified.
...
- **One compact record per finding.** A line violation has one required finding line: `<path>: <actual> <counting basis> (budget <limit>)`; byte and token violations retain `<path>: <actual> <unit>`. Text output may add one configured guidance line; JSON carries the same message fields in the record.
