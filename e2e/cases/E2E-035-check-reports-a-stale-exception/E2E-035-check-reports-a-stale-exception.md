...
An exact-path exception the run's own file set proves is gone accepts nothing.
`check` reports it in a block of its own, named by the registry that holds it
